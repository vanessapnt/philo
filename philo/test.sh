...
commands_needed=("awk" "echo" "date" "sed" "pgrep" "sleep" "valgrind" "git" "mkdir" "make" "nm" "grep" "wc" "cat" "ps" "hostname" "head")

# Vérifier les commandes nécessaires
for command_needed in "${commands_needed[@]}"
do
	if ! command -v $command_needed > /dev/null 2>&1; then
		fatal_error "'$command_needed' command not installed... Aborting."
	fi
done

# Vérifier si 'timeout' est installé, sinon utiliser 'timeout_command'
if ! command -v timeout > /dev/null 2>&1; then
	echo "'timeout' command not installed. Using custom timeout.sh script."
...
test_philosopher_death () {
	...
	(timeout_command 10 "$program_path/$program_name" "${params[@]}" > "$log_file")
...
test_philosopher_meals () {
	...
	(timeout_command 20 "$program_path/$program_name" "${program_params[@]}" > "$log_file")
...
test_valgrind () {
  ...
  local test_num="$8"
  timeout_command 30 valgrind --leak-check=full --errors-for-leak-kinds=all --error-exitcode=1 "$2/$1" $parameters &> "./valgrind_$1.log"
  if [ $? -eq 0 ]; then
...
check_philosophers_nodeath ()
{
	...
	(timeout_command 50 "$program_path/$program_name" "${program_params[@]}" > /dev/null)&
	i=1
    error=0
    while [ $i -lt 40 ];do
        printf "\r[%d...]" $i
...
check_cpu_usage() {
    ...
    timeout_command 15 "$program_path/$program_name" "${params[@]}" &>.debug.txt & pid=$!
...
check_program_arguments() {
    ...
    timeout_command 10 "$program_path/$program_name" "${params[@]}" &> output.txt
...
check_number_of_forks ()
{
    ...
    local params=("${@:3:4}")
	local test_number="$7"
	(timeout_command 10 "$program_path/$program_name" "${params[@]}" &> /dev/null)&
...
check_secure_thread_creation () {
	...
	result=$( (timeout_command 10 ulimit -v 175000; valgrind --leak-check=full --errors-for-leak-kinds=all  "$program_path/$program_name" 10 60 60 60) 2>&1 )
